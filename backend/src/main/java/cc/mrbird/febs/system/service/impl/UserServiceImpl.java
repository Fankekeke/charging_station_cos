...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    @Override
    public void registPharmacy(PharmacyInfo pharmacyInfo, String password) throws Exception {
        User user = new User();
        user.setPassword(MD5Util.encrypt(pharmacyInfo.getCode(), password));
        user.setUsername(pharmacyInfo.getCode());
        user.setCreateTime(new Date());
        user.setStatus(User.STATUS_VALID);
        user.setSsex(User.SEX_UNKNOW);
        user.setAvatar(User.DEFAULT_AVATAR);
